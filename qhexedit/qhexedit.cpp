...
QHexEdit::QHexEdit(QWidget *parent) : QScrollArea(parent)
{
    ...
    connect(qHexEdit_p, SIGNAL(updateText(QString)), this, SIGNAL(updateText(QString)));
    setFocusPolicy(Qt::NoFocus);
}

void QHexEdit::setExternalHighlight(QByteArray hl)
{
    qHexEdit_p->setExternalHighlight(hl);
}

void QHexEdit::gotoAddress(int addr)
{
    qHexEdit_p->gotoAddress(addr);
}
