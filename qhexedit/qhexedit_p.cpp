...
QHexEditPrivate::QHexEditPrivate(QScrollArea *parent) : QWidget(parent)
{
    ...
    connect(_scrollArea->verticalScrollBar(),SIGNAL(valueChanged(int)),this,SLOT(moveVbar(int)));
...
void QHexEditPrivate::moveVbar(int pos) //dirty hack - do not allow reset vscroll then tab changed
{
    int tt =(_scrollArea->verticalScrollBar()->maximum() - _scrollArea->verticalScrollBar()->minimum())/2;
    if (pos == tt)
    {
        _scrollArea->verticalScrollBar()->setValue(_vbarPos);
    }
    else
    {
        _vbarPos = pos;
...
void QHexEditPrivate::gotoAddress(int addr)
{
    setCursorPos(addr*2);
    _scrollArea->ensureVisible(_cursorX, _cursorY + _charHeight/2, 3, _charHeight/2 + 2);
}

void QHexEditPrivate::setExternalHighlight(QByteArray hl)
{
    _exteralHl = hl;
    update();
}
